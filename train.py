...
def get_score(hits, counts, pflag=False):
    ...
    #se = (hits[1] + hits[2] + hits[3] + hits[4]) / (counts[1] + counts[2] + counts[3] + counts[4])
    sp = hits[0] / counts[0]
    sc = (se+sp) / 2.0
    ...
    if pflag:
        ...
        #print("Normal: {}, Crackle: {}, Wheeze: {}, Both: {}".format(hits[0]/counts[0], hits[1]/counts[1], hits[2]/counts[2], hits[3]/counts[3]))
        print("Normal: {}, Rale: {}, Rhonchi: {}, Wheezing: {}".format(hits[0]/counts[0], hits[1]/counts[1], hits[2]/counts[2], hits[3]/counts[3]))
...
class Trainer:
    def __init__(self):
        ...
        #self.net = model_rnn(args.batch_size, 28, 192, 150, 4).cuda()
        self.net = model_transformer(num_classes=4)
        #self.net = model(num_classes=5).cuda()
        ...
        #weights = torch.tensor([3.0, 1.0, 1.0, 1.0, 1.0], dtype=torch.float32)
    ...
    def evaluate(self, net, epoch, iteration):
        ...
        class_hits = [0.0, 0.0, 0.0, 0.0] # normal, rale, rhonchi, wheezing
        class_counts = [0.0, 0.0, 0.0+1e-7, 0.0+1e-7] # normal, rale, rhonchi, wheezing
        running_corrects = 0.0
        denom = 0.0

        #print("val_data_loader : {}".format(self.val_data_loader))
        ...
        print("Val Accuracy: {}".format(running_corrects / denom))
        print("Val Accuracy2: {}".format((running_corrects * 2) / denom))
    ...
    def train(self):
        ...
        for _, epoch in tqdm(enumerate(range(self.args.start_epochs, self.args.epochs))):
            ...
            for i, (image, label) in tqdm(enumerate(self.train_data_loader)):
                ...
                if i % 1000 == self.train_data_loader.__len__()-1 and i != 0:
                    print("---------------------------------------------")
