...
# install anaconda
RUN apt-get update
RUN apt-get install -y wget bzip2 build-essential gcc libsndfile1 libgl1-mesa-glx ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 git mercurial subversion && \
        apt-get clean
...
# if "cannot open cv2", "pip install opencv-python"