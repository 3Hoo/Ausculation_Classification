...
class image_loader(Dataset):
    def __init__(self, data_dir, folds_file, test_fold, train_flag, params_json, input_transform=None, stetho_id=-1, aug_scale=None):
        ...
        for f in files:
            ...
            if pat not in device_patient_list[device_to_id[device]]:
                ...
                # device_patient_list = [[환자1, 환자3, ...], [환자2, 환자4, ...], [], []]
                # device 별 환자 목록
            if pat not in pats:
                pats.append(pat)
                # pats = [환자1, 환자2, 환자3, ...]
        ...
        # fold_file => data 폴더 내의 patient_list_foldwise.txt
        patient_dict = {}
        ...
        filenames, rec_annotations_dict = get_annotations_custom(data_dir) # txt 파일분석
        # filenames => 파일 이름에서 추출한 내용 (확장자 X)
        # rec_annotations_dict => 각 txt 파일에서 추출한 시작시간, 종료시간, 그리고 어떤 클래스인지
        if stetho_id >= 0:
        ...
        for idx, file_name in tqdm(enumerate(self.filenames)):
            data = get_sound_samples_custom(rec_annotations_dict[file_name], file_name, data_dir, self.sample_rate)
            # data = [ filename, (잘린 오디오 파일1, start 시간1, end 시간1, 라벨1), (잘린 오디오 파일2, start 시간2, end 시간2, 라벨2) ]
        ...
        for idx, sample in enumerate(self.cycle_list):
            ...
            # audio_data => (잘린/패딩된 오디오 파일, 라벨, filename, cycle_idx, 잘린 cycle 내에서의 idx, padding 횟수) 의 리스트

        self.device_wise = []
        for idx in range(device_id):
            self.device_wise.append([])
        self.class_probs = np.zeros(4)
        #self.class_probs = np.zeros(5)
        self.identifiers = []
        for idx, sample in enumerate(self.audio_data):
            ...
            # self.device_wise에는 [ [], [], [], [] ] 각 device idx에 맞는 곳에 sample(audio data 튜플)이 들어간다
    ...
    def new_augment_custom(self, scale=1):
        # augment normal
        ...
        for idx in range(aug_nos):
            ...
            if aug_prob < 0.6:
                # noise_i + noise_j
                i = random.randint(0, len(self.classwise_cycle_list[4])-1)
                j = random.randint(0, len(self.classwise_cycle_list[4])-1)
                sample_i = self.classwise_cycle_list[4][i]
                sample_j = self.classwise_cycle_list[4][j]
            elif aug_prob >= 0.6 and aug_prob < 0.8:
                # noise_i + normal_j
                i = random.randint(0, len(self.classwise_cycle_list[4])-1)
                j = random.randint(0, len(self.classwise_cycle_list[0])-1)
            ...
            new_sample = np.concatenate([sample_i[0], sample_j[0]])
            self.cycle_list.append((new_sample, 4, sample_i[2]+'-'+sample_j[2],
                idx, 0))
            self.filenames_with_labels.append(sample_i[2]+'-'+sample_j[2]+'_'+str(idx)+'_4')
        '''
