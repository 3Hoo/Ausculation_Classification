...
def Extract_Annotation_Data_custom(file_name, data_dir) : 
    tokens = file_name.split('_')
    recording_info = pd.DataFrame(data = [tokens], columns = ['Patient Number', 'Recording index', 'Chest location','Acquisition mode','Recording equipment'])
    recording_annotations = pd.read_csv(os.path.join(data_dir, file_name + '.txt'), names = ['Start', 'End', 'normal', 'rale', 'rhonchi', 'wheezing'], delimiter= '\t')
    return recording_info, recording_annotations
...
def get_annotations_custom(data_dir):
    filenames = [s.split('.')[0] for s in os.listdir(data_dir) if '.txt' in s]
    i_list = []
    rec_annotations_dict = {}
    for s in filenames :
        i, a = Extract_Annotation_Data_custom(s, data_dir)
        i_list.append(i)
        rec_annotations_dict[s] = a
...
def get_label_custom(normal, rale, rhonchi, wheezing) : 
    if normal == 1 :
        return 0
    elif rale == 1 :
        return 1
...
def get_sound_samples(recording_annotations, file_name, data_dir, sample_rate):
    ...
    for i in range(len(recording_annotations.index)):
        ...
        # sample_data = [ filename, (잘린 오디오 파일, start 시간, end 시간, 라벨) ]
    return sample_data

def get_sound_samples_custom(recording_annotations, file_name, data_dir, sample_rate):
    sample_data = [file_name]
    # load file with specified sample rate (also converts to mono)
    data, rate = librosa.load(os.path.join(data_dir, file_name+'.wav'), sr=sample_rate)
    #print("Sample Rate", rate)
    ...
    for i in range(len(recording_annotations.index)):
        ...
        audio_chunk = slice_data(start, end, data, rate)
        sample_data.append((audio_chunk, start,end, get_label_custom(normal, rale, rhonchi, wheezing)))
        # sample_data = [ filename, (잘린 오디오 파일, start 시간, end 시간, 라벨) ]
    return sample_data

def floating_point_cut(f, length) :
    a = str(f)
    b1 = a.split('.')[0]
    b2 = a.split('.')[1]
    if len(b2) < length : 
...
def get_sound_samples_for_predict(file_name, data_dir, sample_rate):
    sample_data = [file_name]
    # load file with specified sample rate (also converts to mono)
    ...
    duration = floating_point_cut(duration, 3)
